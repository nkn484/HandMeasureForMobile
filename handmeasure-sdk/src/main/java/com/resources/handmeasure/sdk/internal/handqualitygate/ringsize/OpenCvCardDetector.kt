...
class OpenCvCardDetector(
    ...
    // Perspective distortion and rounded corners can skew aspect; keep this tolerant.
    ...
    override fun detect(frame: FramePacket): CardDetection? {
        ...
        try {
            ...
            // Fallback pass: lower thresholds + morphology to close gaps on textured backgrounds.
            ...
            Log.w("OpenCvCardDetector", "Detect failed: ${t.message}")
            return null
        } finally {
    ...
    private fun bestCandidate(
        ...
        for (contour in contours) {
            ...
            // Don't punish small cards too harshly: normalize area vs the minimum accepted area.
            val areaScore =
    ...
    private fun Double.normalizeAspect(): Double {
        ...
        // Accept portrait or landscape: compare with canonical ratio (> 1).
